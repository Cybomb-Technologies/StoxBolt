...
const getRandomImage = (categoryName = "") => {
  ...
  if (normalizedCat.includes("market") || normalizedCat.includes("stock"))
    images = categoryImages.markets;
  else if (
    normalizedCat.includes("economy") ||
    normalizedCat.includes("finance")
  )
    images = categoryImages.economy;
  else if (normalizedCat.includes("tech") || normalizedCat.includes("digital"))
    images = categoryImages.technology;
  else if (normalizedCat.includes("ipo") || normalizedCat.includes("startup"))
    images = categoryImages.ipo;
...
const transformPostData = (post) => {
  ...
  const getAuthorName = (authorData) => {
    ...
    if (typeof authorData === "object") {
      return (
        authorData.name || authorData.username || authorData.email || "Admin"
      );
    }
    return "Admin";
  };
  ...
  return {
    ...
    summary:
      post.summary ||
      post.excerpt ||
      (getBodyContent(post.body)
        ? getBodyContent(post.body).substring(0, 150) + "..."
        : ""),
...
const HomePage = () => {
  ...
  const metaPropsData = {
    title: "Latest Financial News & Market Updates Platform | StoxBolt",
    description:
      "Real-time financial news, stock market analysis, breaking updates across Indian, US, and global markets. Your trusted source for investment intelligence data.",
    keyword:
      "latest financial news, share and stock market news, investment updates, market analysis, live market news, crypto news",
    image: "https://www.stoxbolt.com/images/logo.png",
    url: "https://stoxbolt.com/",
  };
